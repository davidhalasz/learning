<div class="w-full bg-[#dfe5ea] py-16">
    <div class="container mx-auto">
        <div class="flex justify-center">
            <div class="grid grid-cols-4 w-fit gap-16">
                <div>
                    <h1 class="text-xl mb-4">LOGO CENTER</h1>
                    <p>North Coast Tanácsadó Kft. </br>
                        Cégjegyzékszám: 01-09-183357</br>
                        Adószám: 24805483-2-41</br>
                    </p>
                </div>
                <div>
                    <h1 class="text-xl mb-4">FELNŐTTKÉPZÉS</h1>
                    <ul>
                        <li>Nyilvántartási szám: B/2022/001140</li>
                        <li class="mb-2">Nyilvántartásba véve: 2022.09.16.</li>
                        <li>Ügyfélszolgálat helye:</li>
                        <li class="mb-2">1054 Budapest Báthori utca 3. 1. emelet</li>
                        <li>Ügyfélszolgálat nyitvatartása:</li>
                        <li>Hétfőtől-Péntekig: 8:00-16:30</li>
                        <li>Szombat és Vasárnap: ZÁRVA</li>
                    </ul>
                </div>
                <div>
                    <h1 class="text-xl mb-4">KAPCSOLAT</h1>
                    <ul>
                        <li>Levelezési cím</li>
                        <li class="mb-2">1054 Budapest, Báthori utca 3. I. em.</li>
                        <li>Telefonszám</li>
                        <li class="mb-2">+36 20 599 6833</li>
                        <li>E-mail</li>
                        <li><a href="mailto:info@northcoast.hu">info@northcoast.hu</a></li>
                    </ul>
                </div>
                <div>
                    <h1 class="text-xl mb-4">LINKEK</h1>
                    <ul>
                        <li>Főoldal</li>
                        <li>Rólunk</li>
                        <li>Szolgáltatásaink</li>
                        <li>Képzések</li>
                        <li>Karrier</li>
                        <li>Kapcsolat</li>
                    </ul>
                </div>
            </div>

        </div>
    </div>
</div>
